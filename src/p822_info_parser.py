...
class P822InfoParser:
    ...
    def get_controller_arrays(self, slot):
        arrays = []
        arrays_str_list = self.run_command(f"{self.path_to_cli}\\hpacucli.exe", f"controller slot={slot} array all show")
    ...
    def get_drives_for_array(self, slot, array):
        physical_drives = []
        drives_list_str = self.run_command(f"{self.path_to_cli}\\hpacucli.exe", f"controller slot={slot} array {array} physicaldrive all show")
    ...
    def get_drive_info(self, slot, array, drive):
        drive_info = []
        drive_info_list_str = self.run_command(f"{self.path_to_cli}\\hpacucli.exe", f"controller slot={slot} array {array} physicaldrive {drive} show")

        print(drive_info_list_str)
